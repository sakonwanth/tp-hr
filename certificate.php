...
<div class="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6 min-w-0 max-w-full">
        ...
        <form id="certificate-form" class="native-card tp-native-card tp-native-data-card p-4 sm:p-6 min-w-0" method="POST" action="/api/certificate.php">
            ...
            <div class="tp-native-form-group">
                ...
                <div class="flex flex-col sm:flex-row sm:flex-wrap gap-3">
                    <label class="flex items-center min-h-[48px] cursor-pointer touch-manipulation rounded-[20px] border border-white/10 bg-white/5 px-4 has-[:checked]:border-violet-500/50 has-[:checked]:bg-violet-500/10">
            ...
            <!-- Rush Request -->
            <div class="tp-native-form-group p-4 rounded-[20px] bg-amber-500/10 border border-amber-400/35">
            ...
            <div class="flex flex-col-reverse sm:flex-row gap-3 sm:gap-4 pt-2">
                <a href="certificate.php" class="touch-manipulation flex-1 min-h-[52px] inline-flex items-center justify-center py-3 bg-white/10 hover:bg-white/20 text-white text-center rounded-[20px] transition-colors font-medium">
                    ยกเลิก
                </a>
                <button type="submit" class="touch-manipulation flex-1 min-h-[56px] inline-flex items-center justify-center gap-2 py-3 bg-violet-600 hover:bg-violet-700 text-white rounded-[20px] transition-colors font-semibold border-0">
    ...
    <div class="space-y-4 md:space-y-6 min-w-0">
        ...
        <div class="native-card tp-native-card tp-native-data-card p-4 sm:p-6 min-w-0">
            ...
            <div class="space-y-2">
                <?php foreach ($templates as $tpl): ?>
                <div class="py-3 px-3 rounded-[20px] bg-white/5 border border-white/8">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-4 md:mb-6 min-w-0 max-w-full">
    <a href="certificate.php?action=new" class="native-card tp-native-card tp-native-data-card p-4 sm:p-6 min-w-0 hover:bg-white/[0.07] transition-colors group touch-manipulation border-violet-500/20">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-[20px] bg-violet-600/20 flex items-center justify-center group-hover:bg-violet-600/30 transition-colors">
    ...
    <div class="native-card tp-native-card tp-native-data-card p-4 sm:p-6 min-w-0">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-[20px] bg-amber-500/20 flex items-center justify-center">
    ...
    <div class="native-card tp-native-card tp-native-data-card p-4 sm:p-6 min-w-0">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 rounded-[20px] bg-emerald-500/20 flex items-center justify-center">
...
<div class="native-card tp-native-card tp-native-data-card min-w-0 max-w-full overflow-hidden">
    ...
    <div class="tp-native-empty-state text-center py-12 px-4 rounded-[20px] border border-dashed border-white/15 max-w-none mx-4 my-4">
        ...
        <a href="certificate.php?action=new" class="inline-flex mt-4 min-h-[56px] items-center justify-center px-6 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-[20px] transition-colors touch-manipulation font-semibold">
            ขอหนังสือรับรองใหม่
        </a>
    </div>
    <?php else: ?>
    ...
    <div class="space-y-3 p-3 md:p-4 min-w-0">
        ...
        <div class="p-4 md:p-5 rounded-[20px] bg-white/5 border border-white/8 hover:bg-white/[0.07] transition-colors min-w-0">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 min-w-0">
                <div class="flex items-center gap-4 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <span class="inline-block mt-1 px-2 py-0.5 bg-amber-500/20 text-amber-300 text-xs rounded-[20px] border border-amber-500/30">ด่วน</span>
                        <?php endif; ?>
                    </div>
                </div>
                <div class="flex flex-col gap-2 w-full md:w-auto md:items-end md:shrink-0">
                    <span class="px-3 py-1 rounded-[20px] text-xs font-medium border border-white/10 self-start <?php echo $statusColors[$req['status']] ?? 'bg-gray-500/20 text-gray-400'; ?>">
                    ...
                    tpHrCertificatePrintForm(
                        ...
                        'min-h-[56px] inline-flex items-center justify-center px-4 py-2 bg-violet-600 hover:bg-violet-700 text-white rounded-[20px] transition-colors text-sm font-semibold touch-manipulation w-full sm:w-auto border-0',
                    ...
                    <a href="<?php echo htmlspecialchars($req['file_path']); ?>" target="_blank" rel="noopener noreferrer"
                       class="min-h-[56px] inline-flex items-center justify-center px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-[20px] transition-colors text-sm font-semibold touch-manipulation w-full sm:w-auto">
                    ...
                    <button type="button" onclick="cancelRequest(<?php echo (int)$req['id']; ?>)"
                            class="min-h-[48px] inline-flex items-center justify-center px-4 py-2 bg-red-500/15 hover:bg-red-500/25 border border-red-500/30 text-red-300 rounded-[20px] transition-colors text-sm font-medium touch-manipulation w-full sm:w-auto">
            ...
            <div class="mt-3 p-3 rounded-[20px] bg-red-500/10 border border-red-400/35">
