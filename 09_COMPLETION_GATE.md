...
| Automated browser/UI proof in CI | API / route smoke (`tests/e2e`) optional; **`npm run verify:static-ui`** (**`verify:shell-cache`** + **`verify:ios26-pages`**) — **`?v=`** + **`tp-ios-master-screen`** on ESS/**`hr/*`** · **no substitute** for device matrix (**`AUDIT_04_VIEWPORT`** = static CSS only) |
