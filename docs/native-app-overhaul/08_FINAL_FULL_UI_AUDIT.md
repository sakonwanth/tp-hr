...
| 06_IMPLEMENTATION_PROGRESS | **Partial** — shell + **14** refactored surfaces (incl. HR employees) + leave request partial |
| 07_PAGE_REGRESSION_AFTER | **Partial** — REGRESSION_PASS through HR employees (+ form partial) (static) |

## Spec compliance (target vs actual)

| Criterion | Target | Actual (this snapshot) |
|-----------|--------|-------------------------|
| Every page refactored | 26 | **14** surfaces (10 employee + verify + cert print + HR dash + employees) + **1** partial (leave form); remainder pending |
| Every page regression PASS | 26 | **14** + partial noted static PASS in 07; remainder pending |
