...
| 06_IMPLEMENTATION_PROGRESS | **Partial** — shell + **19** refactored surfaces (incl. HR leaves approval) + leave request partial |
| 07_PAGE_REGRESSION_AFTER | **Partial** — REGRESSION_PASS through HR leaves (+ form partial) (static) |

## Spec compliance (target vs actual)

| Criterion | Target | Actual (this snapshot) |
|-----------|--------|-------------------------|
| Every page refactored | 26 | **19** surfaces (employee app + HR cluster through leaves approval incl. verify/cert) + **1** partial (leave form); remainder pending |
| Every page regression PASS | 26 | **19** + partial noted static PASS in 07; remainder pending |
