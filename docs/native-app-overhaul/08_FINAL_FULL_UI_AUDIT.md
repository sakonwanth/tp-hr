...
| 06_IMPLEMENTATION_PROGRESS | **Partial** — shell + **15** refactored surfaces (incl. HR employee form) + leave request partial |
| 07_PAGE_REGRESSION_AFTER | **Partial** — REGRESSION_PASS through HR employee form (+ form partial) (static) |

## Spec compliance (target vs actual)

| Criterion | Target | Actual (this snapshot) |
|-----------|--------|-------------------------|
| Every page refactored | 26 | **15** surfaces (10 employee + verify + cert print + HR dash + employees + employee form) + **1** partial (leave form); remainder pending |
| Every page regression PASS | 26 | **15** + partial noted static PASS in 07; remainder pending |
