...
| 06_IMPLEMENTATION_PROGRESS | **Partial** — shell + **21** refactored surfaces (incl. HR document requests) + leave request partial |
| 07_PAGE_REGRESSION_AFTER | **Partial** — REGRESSION_PASS through HR documents (+ form partial) (static) |

## Spec compliance (target vs actual)

| Criterion | Target | Actual (this snapshot) |
|-----------|--------|-------------------------|
| Every page refactored | 26 | **21** surfaces (employee app + HR cluster through document requests incl. verify/cert) + **1** partial (leave form); remainder pending |
| Every page regression PASS | 26 | **21** + partial noted static PASS in 07; remainder pending |
