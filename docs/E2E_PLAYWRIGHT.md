...
| Optional HR asserts | **`PLAYWRIGHT_HR_EXPECT_ADMIN=1`** — **`hr/index`** + **`hr/employees`** titles (`authenticated.spec.cjs`, both auth projects) |
...
2. **`tests/e2e/authenticated.spec.cjs`** — **`chromium-auth`** + **`tablet-auth`** (unless **`PLAYWRIGHT_SKIP_TABLET=1`**) use that session — dashboard hero plus employee-route title checks (**`checkin`**, **`leave`**, **`profile`**, **`payslip`**, **`attendance_history`**, **`leave_history`**, **`certificate`**, **`dayoff_schedule`**), optional **`hr/index.php`** / **`hr/employees.php`** titles when **`PLAYWRIGHT_HR_EXPECT_ADMIN=1`**.
...
Sets assertions on **`hr/index.php`** (HR Dashboard) and **`hr/employees.php`** (จัดการพนักงาน).
...
| `protected-routes.spec.cjs` | Guest redirect to **`login.php`** for **`index`**, **`checkin`**, **`leave`**, **`profile`**, **`payslip`**, **`attendance_history`**, **`leave_history`**, **`certificate`**, **`dayoff_schedule`**, **`hr/index`**. |
| `authenticated.spec.cjs` | Logged-in **`index`** hero + employee route titles (see list above); **`hr/index`**, **`hr/employees`** when **`PLAYWRIGHT_HR_EXPECT_ADMIN=1`**. Projects: **`chromium-auth`**, **`tablet-auth`**. |
