...
$enforceLocationCheckin = ($enforceSetting && $enforceSetting['value'] === '1');
...
<main class="content-area p-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="flex flex-col items-center gap-4">
                    ...
                    <div id="location-status" class="text-center">
                        <p class="text-white/50 text-sm">
                            ...
                            <span id="location-msg">กำลังตรวจสอบตำแหน่ง...</span>
                        </p>
                        <?php if ($enforceLocationCheckin): ?>
                        <p class="text-yellow-400 text-xs mt-1">
                            <i class="fas fa-exclamation-triangle mr-1"></i>
                            ต้องอยู่ในพื้นที่ที่กำหนดเท่านั้น
                        </p>
                        <?php endif; ?>
                    </div>
                </div>
        ...
        <div class="space-y-6">
            <!-- Allowed Locations -->
            <?php if ($enforceLocationCheckin && !empty($locations)): ?>
            <div class="glass-card rounded-xl p-6">
                ...
                <div class="space-y-2" id="locations-list">
                    ...
                    <div class="p-3 rounded-lg bg-white/5 location-item" data-lat="<?php echo $loc['latitude']; ?>" data-lng="<?php echo $loc['longitude']; ?>" data-radius="<?php echo $loc['radius_meters'] ?? 100; ?>" data-name="<?php echo htmlspecialchars($loc['name']); ?>">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-white font-medium loc-name"><?php echo htmlspecialchars($loc['name']); ?></p>
                                <p class="text-white/50 text-xs">รัศมี <?php echo number_format($loc['radius_meters'] ?? 100); ?> ม.</p>
                            </div>
                            <span class="location-distance px-2 py-1 rounded text-xs bg-gray-500/20 text-gray-400">
...
<!-- Check-in Modal -->
<div id="checkin-modal" class="fixed inset-0 bg-black/70 backdrop-blur-md z-50 hidden flex items-center justify-center p-4">
    <div class="glass-card rounded-2xl p-6 w-full max-w-md shadow-2xl border border-white/10">
        <!-- Header with Time -->
        <div class="text-center mb-5">
            <div class="text-5xl font-bold text-white/30 mb-3 tracking-wider" id="modal-time">--:--:--</div>
            <div class="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 relative" id="modal-icon-wrapper">
                <div class="absolute inset-0 rounded-full bg-gradient-to-br from-green-500/20 to-green-600/20 animate-pulse"></div>
                <i class="fas fa-sign-in-alt text-green-400 text-3xl relative z-10" id="modal-icon"></i>
            </div>
            <h3 class="text-xl font-bold text-white" id="modal-title">ลงเวลาเข้างาน</h3>
            <p class="text-white/50 text-sm mt-1" id="modal-subtitle">ถ่ายรูปเพื่อยืนยันตัวตน</p>
        </div>
        
        <!-- Camera Preview -->
        <div class="relative mb-5 rounded-xl overflow-hidden bg-black/50 aspect-video border border-white/10" id="camera-container">
            ...
            <!-- Photo overlay badge -->
            <div id="photo-timestamp" class="hidden absolute bottom-2 left-2 right-2 bg-black/60 backdrop-blur-sm rounded-lg px-3 py-2 text-white text-xs">
                <div class="flex items-center justify-between">
                    <span id="photo-time-text"></span>
                    <span id="photo-location-text" class="text-green-400"></span>
                </div>
            </div>
        </div>
        
        <!-- Location Info - Enhanced -->
        <div class="rounded-xl overflow-hidden mb-5" id="location-info">
            <div class="p-4 bg-gradient-to-r from-white/5 to-white/10 border border-white/10 rounded-xl">
                <div class="flex items-start gap-3">
                    <div class="w-10 h-10 rounded-full bg-green-500/20 flex items-center justify-center flex-shrink-0" id="loc-icon-wrapper">
                        <i class="fas fa-map-marker-alt text-green-400" id="loc-icon"></i>
                    </div>
                    <div class="flex-1 min-w-0">
                        <div id="location-status-text" class="flex items-center gap-2 mb-1">
                            <span class="text-white/60 text-sm">กำลังระบุตำแหน่ง...</span>
                        </div>
                        <div id="location-name-display" class="hidden">
                            <p class="text-white font-semibold text-base truncate" id="loc-name-text">-</p>
                            <p class="text-white/50 text-xs mt-0.5" id="loc-distance-text">-</p>
                        </div>
                    </div>
                    <div id="loc-badge" class="hidden px-2.5 py-1 rounded-full text-xs font-medium bg-green-500/20 text-green-400 flex-shrink-0">
                        <i class="fas fa-check-circle mr-1"></i>อยู่ในพื้นที่
                    </div>
                </div>
            </div>
            ...
            <div id="outside-reason-wrap" class="hidden mt-3">
                <label class="block text-yellow-300 text-sm mb-1">
                    <i class="fas fa-exclamation-triangle mr-1"></i>
                    เหตุผลการลงเวลานอกสถานที่ (จำเป็นต้องกรอก)
                </label>
                <textarea id="outside-reason" class="input-field" rows="3" placeholder="ระบุเหตุผลที่ลงเวลานอกสถานที่ที่กำหนด"></textarea>
                <p class="text-white/50 text-xs mt-1">คำขอนี้จะถูกส่งให้ผู้มีอำนาจอนุมัติก่อนบันทึกเวลา</p>
            </div>
        </div>
        
        <!-- Buttons -->
        <div class="flex gap-3">
            <button type="button" onclick="closeCheckinModal()" class="flex-1 py-3.5 bg-white/5 hover:bg-white/10 text-white rounded-xl transition-all border border-white/10 font-medium">
                ยกเลิก
            </button>
            <button type="button" id="btn-capture" onclick="capturePhoto()" class="flex-1 py-3.5 bg-gradient-to-r from-violet-600 to-violet-700 hover:from-violet-500 hover:to-violet-600 text-white rounded-xl transition-all shadow-lg shadow-violet-500/25 font-medium">
                <i class="fas fa-camera mr-2"></i>ถ่ายรูป
            </button>
            <button type="button" id="btn-confirm" onclick="confirmCheckin()" class="flex-1 py-3.5 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-400 hover:to-green-500 text-white rounded-xl transition-all shadow-lg shadow-green-500/25 font-medium hidden">
...
// Configuration
const enforceLocation = <?php echo $enforceLocationCheckin ? 'true' : 'false'; ?>;
const allowedLocations = <?php echo json_encode(array_values(array_map(function($l) {
    return ['name' => $l['name'], 'lat' => (float)$l['latitude'], 'lng' => (float)$l['longitude'], 'radius' => (int)($l['radius_meters'] ?? 100)];
}, $locations))); ?>;
...
function checkUserLocation() {
    ...
    let withinLocationName = null;
    
    // Update each location distance display
    document.querySelectorAll('.location-item').forEach(item => {
        const lat = parseFloat(item.dataset.lat);
        const lng = parseFloat(item.dataset.lng);
        const radius = parseInt(item.dataset.radius) || 100;
        const name = item.dataset.name || item.querySelector('.loc-name')?.textContent || 'Unknown';
        const distance = calcDistance(userLatitude, userLongitude, lat, lng);
        const distSpan = item.querySelector('.location-distance');
        
        if (distance <= radius) {
            isWithinAllowedLocation = true;
            withinLocationName = name;
            distSpan.className = 'location-distance px-2 py-1 rounded text-xs bg-green-500/20 text-green-400';
            distSpan.innerHTML = '<i class="fas fa-check-circle mr-1"></i>อยู่ในพื้นที่';
        } else {
            distSpan.className = 'location-distance px-2 py-1 rounded text-xs bg-gray-500/20 text-gray-400';
            distSpan.textContent = formatDistance(distance);
        }
    ...
    if (isWithinAllowedLocation) {
        msgEl.innerHTML = '<i class="fas fa-check-circle text-green-400 mr-1"></i> ' + withinLocationName;
        msgEl.className = 'text-green-400';
    } else if (enforceLocation) {
        msgEl.innerHTML = '<i class="fas fa-times-circle text-red-400 mr-1"></i> ไม่อยู่ในพื้นที่ที่กำหนด';
        msgEl.className = 'text-red-400';
    } else {
        msgEl.innerHTML = '<i class="fas fa-check-circle text-green-400 mr-1"></i> พร้อมลงเวลา';
        msgEl.className = 'text-green-400';
    }

    toggleOutsideReasonInput();
}
...
navigator.geolocation.getCurrentPosition(
    ...
    (error) => {
        document.getElementById('location-msg').innerHTML = '<i class="fas fa-exclamation-triangle text-yellow-400 mr-1"></i> ไม่สามารถระบุตำแหน่งได้';
    },
...
function updateModalLocation() {
    ...
    const nameText = document.getElementById('loc-name-text');
    const distText = document.getElementById('loc-distance-text');
    const badge = document.getElementById('loc-badge');
    const iconWrapper = document.getElementById('loc-icon-wrapper');
    const icon = document.getElementById('loc-icon');
    const hiddenName = document.getElementById('location-name');
    
    if (!userLatitude || !userLongitude) {
        statusText.innerHTML = '<span class="text-white/60 text-sm"><i class="fas fa-spinner fa-spin mr-2"></i>กำลังระบุตำแหน่ง...</span>';
    ...
    if (isWithinAllowedLocation && nearestLocation) {
        // Within allowed location
        nameText.textContent = nearestLocation;
        distText.textContent = 'ระยะห่าง ' + formatDistance(nearestDistance);
        hiddenName.value = nearestLocation;
        badge.classList.remove('hidden');
        badge.className = 'px-2.5 py-1 rounded-full text-xs font-medium bg-green-500/20 text-green-400 flex-shrink-0';
        badge.innerHTML = '<i class="fas fa-check-circle mr-1"></i>อยู่ในพื้นที่';
        iconWrapper.className = 'w-10 h-10 rounded-full bg-green-500/20 flex items-center justify-center flex-shrink-0';
        icon.className = 'fas fa-map-marker-alt text-green-400';
    } else if (nearestLocation) {
        // Not within but has nearest
        nameText.textContent = nearestLocation;
        distText.textContent = 'ใกล้ที่สุด - ' + formatDistance(nearestDistance);
        hiddenName.value = nearestLocation;
        
        if (enforceLocation) {
            badge.classList.remove('hidden');
            badge.className = 'px-2.5 py-1 rounded-full text-xs font-medium bg-red-500/20 text-red-400 flex-shrink-0';
            badge.innerHTML = '<i class="fas fa-times-circle mr-1"></i>นอกพื้นที่';
            iconWrapper.className = 'w-10 h-10 rounded-full bg-red-500/20 flex items-center justify-center flex-shrink-0';
            icon.className = 'fas fa-map-marker-alt text-red-400';
        } else {
            badge.classList.remove('hidden');
            badge.className = 'px-2.5 py-1 rounded-full text-xs font-medium bg-blue-500/20 text-blue-400 flex-shrink-0';
            badge.innerHTML = '<i class="fas fa-check mr-1"></i>พร้อม';
            iconWrapper.className = 'w-10 h-10 rounded-full bg-blue-500/20 flex items-center justify-center flex-shrink-0';
            icon.className = 'fas fa-map-marker-alt text-blue-400';
        }
    } else {
        // No locations defined
        nameText.textContent = 'ระบุตำแหน่งสำเร็จ';
        distText.textContent = userLatitude.toFixed(6) + ', ' + userLongitude.toFixed(6);
        badge.classList.remove('hidden');
        badge.className = 'px-2.5 py-1 rounded-full text-xs font-medium bg-green-500/20 text-green-400 flex-shrink-0';
        badge.innerHTML = '<i class="fas fa-check-circle mr-1"></i>พร้อม';
        iconWrapper.className = 'w-10 h-10 rounded-full bg-green-500/20 flex items-center justify-center flex-shrink-0';
        icon.className = 'fas fa-map-marker-alt text-green-400';
    }
}
...
function startCheckin(type) {
    ...
    if (type === 'in') {
        ...
        icon.className = 'fas fa-sign-in-alt text-green-400 text-3xl relative z-10';
        iconWrapper.querySelector('.absolute').className = 'absolute inset-0 rounded-full bg-gradient-to-br from-green-500/20 to-green-600/20 animate-pulse';
    } else {
        title.textContent = 'ลงเวลาออกงาน';
        icon.className = 'fas fa-sign-out-alt text-blue-400 text-3xl relative z-10';
        iconWrapper.querySelector('.absolute').className = 'absolute inset-0 rounded-full bg-gradient-to-br from-blue-500/20 to-blue-600/20 animate-pulse';
    }
...
function resetModal() {
    photoData = null;
    document.getElementById('outside-reason').value = '';
    document.getElementById('outside-reason-wrap').classList.add('hidden');
    document.getElementById('captured-photo').classList.add('hidden');
    document.getElementById('camera-preview').classList.remove('hidden');
    document.getElementById('btn-capture').classList.remove('hidden');
    document.getElementById('btn-confirm').classList.add('hidden');
    document.getElementById('photo-timestamp').classList.add('hidden');
}
...
function toggleOutsideReasonInput() {
    const wrap = document.getElementById('outside-reason-wrap');
    if (enforceLocation && !isWithinAllowedLocation) {
...
function capturePhoto() {
    ...
    const timestamp = document.getElementById('photo-timestamp');
    const timeText = document.getElementById('photo-time-text');
    const locText = document.getElementById('photo-location-text');
    ...
    locText.textContent = nearestLocation || '';
...
async function confirmCheckin() {
    ...
    const outsideReason = (document.getElementById('outside-reason').value || '').trim();

    if (enforceLocation && !isWithinAllowedLocation && outsideReason.length < 5) {
    ...
    try {
        ...
        if (result.success) {
            ...
            if (result.pending_approval) {
                showToast(result.message || 'ส่งคำขอเรียบร้อย รออนุมัติ', 'success');
            } else {
